...
async def test_user_creation(db_session):
    ...
    user = User(telegram_user_id=123456789, telegram_username="test_user")
    db_session.add(user)
...
async def test_telegram_channel_creation(db_session):
    ...
    channel = TelegramChannel(
        user_id=user.id, channel_id=-1001234567890, channel_username="test_channel", channel_title="Test Channel"
    )
...
async def test_max_channel_creation(db_session):
    ...
    max_channel = MaxChannel(user_id=user.id, channel_id="test_max_channel", channel_title="Test MAX Channel")
...
async def test_crossposting_link_creation(db_session):
    ...
    tg_channel = TelegramChannel(user_id=user.id, channel_id=-1001234567890, channel_title="TG Channel")
    max_channel = MaxChannel(user_id=user.id, channel_id="max_channel", channel_title="MAX Channel")
    ...
    link = CrosspostingLink(user_id=user.id, telegram_channel_id=tg_channel.id, max_channel_id=max_channel.id, is_enabled=True)
    db_session.add(link)
    await db_session.commit()
