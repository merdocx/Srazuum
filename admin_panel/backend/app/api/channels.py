...
async def get_telegram_channels(
    ...
    try:
        ...
        if search:
            query = query.where(
                (TelegramChannel.channel_title.ilike(f"%{search}%")) | (TelegramChannel.channel_username.ilike(f"%{search}%"))
            )
        ...
        # Данные с пагинацией
        query = (
            query.order_by(TelegramChannel.bot_added_at.desc() if TelegramChannel.bot_added_at else TelegramChannel.id.desc())
            .offset(skip)
            .limit(limit)
        )
        ...
        for channel in channels:
            links_count = await db.execute(
                select(func.count(CrosspostingLink.id)).where(CrosspostingLink.telegram_channel_id == channel.id)
            )
        ...
        return {"total": total, "skip": skip, "limit": limit, "data": channels_data}
...
async def get_telegram_channel(
    request: Request, channel_id: int, current_admin: Admin = Depends(get_current_admin), db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        # Получаем связи
        links_result = await db.execute(select(CrosspostingLink).where(CrosspostingLink.telegram_channel_id == channel_id))
...
async def get_max_channels(
    ...
    try:
        ...
        if search:
            query = query.where(
                (MaxChannel.channel_title.ilike(f"%{search}%")) | (MaxChannel.channel_username.ilike(f"%{search}%"))
            )
        ...
        # Данные с пагинацией
        query = (
            query.order_by(MaxChannel.bot_added_at.desc() if MaxChannel.bot_added_at else MaxChannel.id.desc())
            .offset(skip)
            .limit(limit)
        )
        result = await db.execute(query)
        ...
        for channel in channels:
            links_count = await db.execute(
                select(func.count(CrosspostingLink.id)).where(CrosspostingLink.max_channel_id == channel.id)
            )
            links = links_count.scalar() or 0
...
async def get_max_channel(
    ...
    try:
        result = await db.execute(select(MaxChannel).where(MaxChannel.id == channel_id))
        channel = result.scalar_one_or_none()
        ...
        # Получаем связи
        links_result = await db.execute(select(CrosspostingLink).where(CrosspostingLink.max_channel_id == channel_id))
        links = links_result.scalars().all()
