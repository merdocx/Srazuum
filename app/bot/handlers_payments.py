...
async def format_subscription_info(link: CrosspostingLink, telegram_channel: Optional[TelegramChannel], max_channel: Optional[MaxChannel]) -> str:
    ...
    async with async_session_maker() as session:
        ...
        if not links:
            await message.answer(
                ...
                "Создайте связь через команду /create_link",
        ...
        if active_links:
            ...
            for link in active_links:
                ...
                max_result = await session.execute(
                    select(MaxChannel).where(MaxChannel.id == link.max_channel_id)
                )
...
async def cmd_pay_link(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        result = await session.execute(
            select(CrosspostingLink)
            .where(CrosspostingLink.id == link_id, CrosspostingLink.user_id == user.id)
        )
