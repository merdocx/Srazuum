...
def get_link_detail_keyboard(link_id: int, is_enabled: bool, can_enable: bool = True) -> ReplyKeyboardMarkup:
    """
    ...
    Args:
        ...
        can_enable: Можно ли включить связь (на основе статуса подписки)
    ...
    # Если связь не включена и нельзя включить - не показываем кнопку включения
...
def get_help_keyboard() -> ReplyKeyboardMarkup:
    ...
    Returns:
        ReplyKeyboardMarkup с кнопками помощи
    """
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="💬 Связаться с поддержкой")],
            [KeyboardButton(text="❓ Как создать канал в MAX?")],
