...
async def message_support(message: Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=keyboard)
    await message.answer("Выберите действие:", reply_markup=get_help_keyboard())


@router.message(F.text == "❓ Как создать канал в MAX?")
async def message_how_to_create_max_channel(message: Message, state: FSMContext):
    """Обработчик кнопки инструкции по созданию канала в MAX."""
    await state.clear()

    text = (
        "❓ Как создать канал в MAX?\n\n"
        "1️⃣ Убедитесь, что у вас есть канал или сообщество в Telegram, Вконтакте, ОК, Дзене с пометкой А+ "
        "(наличие регистрации в РКН при превышении 10000 подписчиков).\n\n"
        "2️⃣ Откройте https://max.ru/channel_bot и следуйте инструкциям.\n\n"
        "Если у вас нет канала или сообщества с пометкой А+, но есть зарегистрированное юр. лицо в МСП.РФ, "
        "то откройте https://business.max.ru/ и следуйте инструкциям."
    )
    await message.answer(text, reply_markup=get_help_keyboard(), disable_web_page_preview=True)
...
@router.message(AddChannelStates.waiting_email, F.text == "❌ Отмена")
...
async def message_add_channel(message: Message, state: FSMContext):
    ...
    # Проверяем, есть ли e-mail у пользователя
    if user.email:
        # E-mail уже есть, переходим к выбору Telegram-канала
        text = (
            "📋 Создание связи каналов\n\n"
            "⚠️ ВАЖНО! Перед началом убедитесь, что:\n\n"
            "1. ✅ Бот (@srazuum\\_bot) добавлен в ваш Telegram-канал в качестве администратора\n"
        ...
        # E-mail нет, запрашиваем его
        text = (
            "📋 Создание связи каналов\n\n"
            "Для продолжения необходимо указать ваш e-mail адрес.\n\n"
            "📧 Пожалуйста, отправьте ваш e-mail:"
        )
        await message.answer(text, reply_markup=get_cancel_keyboard())
        await state.set_state(AddChannelStates.waiting_email)
        logger.info("add_channel_email_requested", user_id=message.from_user.id)


@router.message(AddChannelStates.waiting_email)
async def process_email_input(message: Message, state: FSMContext):
    ...
    if not re.match(email_pattern, email):
        await message.answer(
            "❌ Неверный формат e-mail адреса.\n\n" "📧 Пожалуйста, отправьте корректный e-mail адрес:",
            reply_markup=get_cancel_keyboard(),
        )
        logger.warning("invalid_email_format", user_id=message.from_user.id, email=email)
        return

    # Сохраняем e-mail в базу данных
    try:
        user = await get_or_create_user(message.from_user.id, message.from_user.username)
        async with async_session_maker() as session:
            ...
            if db_user:
                ...
                logger.info("email_saved", user_id=user.id, telegram_user_id=message.from_user.id, email=email)

        # Переходим к выбору Telegram-канала
        text = (
            "📋 Создание связи каналов\n\n"
            "⚠️ ВАЖНО! Перед началом убедитесь, что:\n\n"
            "1. ✅ Бот (@srazuum\\_bot) добавлен в ваш Telegram-канал в качестве администратора\n"
        ...
        await state.set_state(AddChannelStates.waiting_telegram_channel)
        logger.info("add_channel_started_after_email", user_id=message.from_user.id)
    except Exception as e:
        logger.error("error_saving_email", user_id=message.from_user.id, error=str(e), exc_info=True)
        await message.answer(
            "❌ Произошла ошибка при сохранении e-mail. Попробуйте позже или обратитесь в поддержку.",
            reply_markup=get_cancel_keyboard(),
        )
...
async def process_telegram_channel(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        if not telegram_channel:
            ...
            # Канал уже существует - обновляем его данные (название, username)
            logger.info(
                f"telegram_channel_exists_updating: channel_id={telegram_channel.channel_id}, "
                f"old_title={telegram_channel.channel_title}, new_title={channel_title}, "
                f"old_username={telegram_channel.channel_username}, new_username={channel_username}"
            )
            telegram_channel.channel_title = channel_title
            if channel_username:
                telegram_channel.channel_username = channel_username
            await session.commit()
            await session.refresh(telegram_channel)
...
async def process_max_channel(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        # Сначала получаем actual_channel_id из API, чтобы проверить существование канала
        # Временные переменные для хранения данных канала
        ...
        # Теперь проверяем, существует ли канал с таким actual_channel_id
        result = await session.execute(select(MaxChannel).where(MaxChannel.channel_id == actual_channel_id))
        max_channel = result.scalar_one_or_none()

        if not max_channel:
            # Валидация входных данных
            try:
                max_input = MaxChannelInput(channel_id=actual_channel_id)
            except Exception as e:
                logger.warning("validation_error", error=str(e))
            ...
            # Канал уже существует - обновляем его данные (название, username)
            logger.info(
                f"max_channel_exists_updating: channel_id={max_channel.channel_id}, "
                f"old_title={max_channel.channel_title}, new_title={channel_title}, "
                f"old_username={max_channel.channel_username}, new_username={channel_username}"
            )
            max_channel.channel_title = channel_title
            if channel_username:
                max_channel.channel_username = channel_username
            await session.commit()
            await session.refresh(max_channel)
        ...
        try:
            ...
            if user.is_vip:
                ...
                # Последующие связи - требуют оплаты
                # Создаем платеж
                from app.bot.handlers_payments import process_pay_link
                ...
                await process_pay_link(message.from_user.id, crossposting_link.id, message, state)
...
async def show_link_detail(message: Message, state: FSMContext, link_id: int):
    ...
    async with async_session_maker() as session:
        ...
        if link.subscription_status == "vip":
            ...
            subscription_type_text = "Платная"
        else:
            subscription_type_text = "Не оплачено"
        ...
        # Определяем, можно ли включить связь
        can_enable = False
        if link.is_first_link:
            can_enable = True
        elif link.subscription_status in ("active", "free_trial", "vip"):
            can_enable = True
        elif link.payment_status == "succeeded":
            can_enable = True

        # Создаем комбинированную клавиатуру: ReplyKeyboardMarkup + InlineKeyboardMarkup
        reply_keyboard = get_link_detail_keyboard(link_id, link.is_enabled, can_enable)
...
async def cmd_enable(message: Message):
    ...
    async with async_session_maker() as session:
        ...
        # Проверяем, можно ли включить связь
        can_enable = False
        if link.is_first_link:
            can_enable = True
        elif link.subscription_status in ("active", "free_trial", "vip"):
        ...
        if not can_enable:
            await message.answer(
                f"❌ Невозможно включить связь #{link_id}: подписка не оплачена.\n\n"
                "Для активации связи необходимо оплатить подписку."
            )
            return
...
async def message_enable(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        # Проверяем, можно ли включить связь
        # Связь можно включить только если:
        # 1. Это первая бесплатная связь ИЛИ
        # 2. Подписка активна (active, free_trial, vip) ИЛИ
        # 3. Была успешная оплата
        can_enable = False
        if link.is_first_link:
            can_enable = True
        elif link.subscription_status in ("active", "free_trial", "vip"):
        ...
        if not can_enable:
            await message.answer(
                "❌ Невозможно включить связь: подписка не оплачена.\n\n"
                "Для активации связи необходимо оплатить подписку. Используйте кнопку '💳 Оплатить подписку' в деталях связи."
            )
            return
        ...
        # Определяем, можно ли включить связь (всегда True после включения)
        keyboard = get_link_detail_keyboard(link_id, True, True)
        await message.answer(text, reply_markup=keyboard)
        logger.info("link_enabled", link_id=link_id, user_id=user.id)
...
async def message_disable(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        # Определяем, можно ли включить связь после отключения
        can_enable = False
        if link.is_first_link:
            can_enable = True
        elif link.subscription_status in ("active", "free_trial", "vip"):
        ...
        keyboard = get_link_detail_keyboard(link_id, False, can_enable)
        await message.answer(text, reply_markup=keyboard)
        logger.info("link_disabled", link_id=link_id, user_id=user.id)
...
# Универсальный обработчик для состояний, которые требуют проверки внутри
# Обрабатывает: LinkManagementStates.viewing_channels_list
# PaymentStates.waiting_email удален
# ВАЖНО: Этот обработчик должен быть зарегистрирован ПОСЛЕ всех специфичных обработчиков
# ВАЖНО: Этот обработчик НЕ должен перехватывать сообщения, если они уже обработаны другими роутерами
@router.message()
async def universal_state_handler(message: Message, state: FSMContext):
    """Универсальный обработчик для состояний с проверкой внутри."""
    current_state = await state.get_state()
    state_key = state.key if hasattr(state, "key") else None

    # Проверяем состояние напрямую в storage, так как оно может быть потеряно в FSMContext
    storage_state = None
    storage_data = None
    if hasattr(state, "storage") and hasattr(state, "key"):
        storage_state = await state.storage.get_state(key=state_key)
        storage_data = await state.storage.get_data(key=state_key)

    # PaymentStates удален

    logger.info(
        f"universal_handler_check: user_id={message.from_user.id}, text={message.text}, "
        f"current_state={current_state}, storage_state={storage_state}, state_key={state_key}"
    )

    # Обработка выбора связи из списка УДАЛЕНА - есть специфичный обработчик message_link_selected
    # который обрабатывает LinkManagementStates.viewing_channels_list с фильтром F.text.startswith("✅") | F.text.startswith("❌")
    # Этот обработчик регистрируется раньше и имеет более специфичный фильтр, поэтому будет вызван первым
    # Универсальный обработчик здесь не нужен для этой функциональности
