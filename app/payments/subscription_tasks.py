    ...
    async with async_session_maker() as session:
        ...
        result = await session.execute(
            ...
            .where(
                ...
                CrosspostingLink.is_enabled == True,
                or_(
                    CrosspostingLink.subscription_end_date.isnot(None),
                    CrosspostingLink.free_trial_end_date.isnot(None)
                )
            )
        )
                ...
                if timedelta(minutes=0) < time_until_expiry <= interval + timedelta(minutes=5):
                    ...
                    try:
                        ...
                        logger.info(
                            ...
                            time_until_expiry=time_text
                        )
                        # Отправляем только одно уведомление за раз
                        break
                    except Exception as e:
                        logger.error(
                            "renewal_notification_failed",
                            link_id=link.id,
                            user_id=link.user_id,
                            error=str(e)
                        )
