...
def create_payment(link_id: int, user_id: int, amount: Optional[float] = None, email: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        email: E-mail пользователя для отправки чека (опционально)
    ...
    # Убеждаемся, что Configuration использует актуальные данные
    Configuration.account_id = settings.yookassa_shop_id
    Configuration.secret_key = settings.yookassa_secret_key
    ...
    # Добавляем receipt с e-mail для отправки чека, если e-mail указан
    if email:
        ...
        logger.info(f"receipt_added_to_payment: email={email}, link_id={link_id}")
