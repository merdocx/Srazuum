...
class MaxAPIClient:
    ...
    async def get_bot_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await retry_with_backoff(self.client.get, "/me")
    ...
    async def send_message(self, chat_id: str, text: str, parse_mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await retry_with_backoff(self.client.post, f"/messages?chat_id={chat_id_value}", json=data)
            response.raise_for_status()
            result = response.json()
    ...
    async def upload_file(self, file_path: str, file_type: str = "image") -> str:
        ...
        try:
            ...
            upload_response = await retry_with_backoff(self.client.post, f"/uploads?type={file_type}")
            ...
            with open(file_path, "rb") as f:
                ...
                try:
                    upload_file_response = await upload_client.post(
                        upload_url, files=files, headers={"Authorization": self.token}
                    )
                    ...
                    if token_from_response:
                            ...
                            try:
                                ...
                                if json_match:
                                    ...
                                    if token_from_response:
                                        ...
                                        raise APIError(
                                            f"Ответ от CDN не является JSON и token не найден: {response_text[:200]}"
                                        )
                        else:
                            upload_result = upload_file_response.json()
                        ...
                        if not token:
                            ...
                            if (
                                "image_invalid_format" in str(error_code).lower()
                                or "image_invalid_format" in str(error_data).lower()
                                or "image_invalid_format" in str(error_msg).lower()
                            ):
                                raise APIError(
                                    "IMAGE_INVALID_FORMAT: Стикер не поддерживается MAX API", response=upload_result
                                )
    ...
    async def send_photo(
        ...
        if local_file_path:
            try:
                ...
                data = {"text": text, "attachments": [{"type": "image", "payload": {"token": token}}]}
                if parse_mode:
                    # MAX API использует "format" вместо "parse_mode"
                    data["format"] = parse_mode
        ...
        text = caption or ""  # Пустая строка, если нет caption
        data = {"text": text, "content": {"type": "image", "src": photo_url}}
    ...
    async def send_video(
        ...
        if local_file_path:
            try:
                ...
                # Формируем запрос с attachments и payload.token
                text = caption or ""  # Пустая строка, если нет caption
                data = {"text": text, "attachments": [{"type": "video", "payload": {"token": token}}]}
                if parse_mode:
                    # MAX API использует "format" вместо "parse_mode"
                    data["format"] = parse_mode
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                "invalid_json_response_for_video",
                                chat_id=chat_id,
                        ...
                        if e.response:
                            try:
                                ...
                                if "attachment.not.ready" in error_msg.lower() or "not.ready" in error_msg.lower():
                                    if attempt < max_retries - 1:
                                        logger.warning(
                                            f"video_attachment_not_ready_retry", attempt=attempt + 1, delay=retry_delay
                                        )
                                        await asyncio.sleep(retry_delay)
                                        retry_delay *= 2
                                        continue
    ...
    async def send_document(
        ...
        if local_file_path:
            try:
                ...
                text = caption or ""  # Пустая строка, если нет caption
                data = {"text": text, "attachments": [{"type": "file", "payload": {"token": token}}]}
                if parse_mode:
                    # MAX API использует "format" вместо "parse_mode"
                    data["format"] = parse_mode
                    ...
                    try:
                        ...
                        if "error" in result or "errors" in result:
                            ...
                            if "attachment.not.ready" in error_msg.lower() or "not.ready" in error_msg.lower():
                                if attempt < max_retries - 1:
                                    logger.warning(
                                        f"document_attachment_not_ready_retry", attempt=attempt + 1, delay=retry_delay
                                    )
                                    await asyncio.sleep(retry_delay)
                                    retry_delay *= 2  # Увеличиваем задержку
                                    continue
    ...
    async def send_sticker(self, chat_id: str, sticker_url: str, local_file_path: Optional[str] = None) -> Dict[str, Any]:
        """
            ...
            try:
                try:
                    ...
                    if upload_response:
                        try:
                            ...
                            if isinstance(error_data, dict):
                                ...
                                if (
                                    "image_invalid_format" in error_code.lower()
                                    or "image_invalid_format" in str(error_data_content).lower()
                                ):
                                    logger.warning(
                                        "sticker_format_not_supported",
                    ...
                    if "image_invalid_format" in error_msg or "invalid format" in error_msg:
                        logger.warning(
                            "sticker_format_not_supported", file_path=local_file_path, chat_id=chat_id, error=str(upload_error)
                        )
                    ...
                    try:
                        ...
                        if "error" in result or "errors" in result:
                            ...
                            if "attachment.not.ready" in error_msg.lower() or "not.ready" in error_msg.lower():
                                if attempt < max_retries - 1:
                                    logger.warning(
                                        f"sticker_attachment_not_ready_retry", attempt=attempt + 1, delay=retry_delay
                                    )
                                    await asyncio.sleep(retry_delay)
                                    retry_delay *= 2
                                    continue
    ...
    async def send_photos(
        self, chat_id: str, local_file_paths: List[str], caption: Optional[str] = None, parse_mode: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i in range(0, len(local_file_paths), batch_size):
                ...
                batch_tokens = await asyncio.gather(*[self.upload_file(file_path, "image") for file_path in batch])
            ...
            processing_delay = min(settings.media_processing_delay_photo * (1 + len(tokens) * 0.1), 10.0)  # Максимум 10 секунд
            await asyncio.sleep(processing_delay)

            # Формируем запрос с массивом attachments
            text = caption or ""  # Пустая строка, если нет caption
            data = {"text": text, "attachments": [{"type": "image", "payload": {"token": token}} for token in tokens]}
            if parse_mode:
                # MAX API использует "format" вместо "parse_mode"
                data["format"] = parse_mode
    ...
    async def send_videos(
        ...
        try:
            ...
            for i in range(0, len(local_file_paths), batch_size):
                batch = local_file_paths[i : i + batch_size]
                # Параллельная загрузка батча
                batch_tokens = await asyncio.gather(*[self.upload_file(file_path, "video") for file_path in batch])
                tokens.extend(batch_tokens)
            ...
            processing_delay = min(
                settings.media_processing_delay_video * (1 + len(tokens) * 0.15), 15.0  # Максимум 15 секунд для видео
            )
            await asyncio.sleep(processing_delay)

            # Формируем запрос с массивом attachments
            text = caption or ""  # Пустая строка, если нет caption
            data = {"text": text, "attachments": [{"type": "video", "payload": {"token": token}} for token in tokens]}
            if parse_mode:
                # MAX API использует "format" вместо "parse_mode"
                data["format"] = parse_mode
                    ...
                    if e.response:
                        try:
                            ...
                            if "attachment.not.ready" in error_msg.lower() or "not.ready" in error_msg.lower():
                                if attempt < max_retries - 1:
                                    logger.warning(
                                        f"videos_attachment_not_ready_retry", attempt=attempt + 1, delay=retry_delay
                                    )
                                    await asyncio.sleep(retry_delay)
                                    retry_delay *= 2
                                    continue
    ...
    async def get_chat(self, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                response = await retry_with_backoff(self.client.get, f"/chats/{numeric_id}")
            except (ValueError, TypeError):
                # Если не числовой, используем как есть
                response = await retry_with_backoff(self.client.get, f"/chats/{chat_id}")
            response.raise_for_status()
            result = response.json()
    ...
    async def get_available_chats(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await retry_with_backoff(self.client.get, "/chats")
            response.raise_for_status()
            result = response.json()
